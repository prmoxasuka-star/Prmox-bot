...
async function attpCommand(sock, chatId, message) {
    ...
    if (!text) {
        ...
        await sock.sendMessage(chatId, { text: '❕ 𝙿𝚕𝚎𝚊𝚜𝚎 𝚙𝚛𝚘𝚟𝚒𝚍𝚎 𝚝𝚎𝚡𝚝 𝚊𝚏𝚝𝚎𝚛 𝚝𝚑𝚎 .𝚊𝚝𝚝𝚙 𝚌𝚘𝚖𝚖𝚊𝚗𝚍...' }, { quoted: message });
        return;
    }

    try {
        const mp4Buffer = await renderBlinkingVideoWithFfmpeg(text);
        const webpPath = await writeExifVid(mp4Buffer, { packname: '𝙿𝚁𝙼𝙾✗' });
        ...
        await sock.sendMessage(chatId, { text: '⚠️ 𝙵𝚊𝚒𝚕𝚎𝚍 𝚝𝚘 𝚐𝚎𝚗𝚎𝚛𝚊𝚝𝚎 𝚝𝚑𝚎 𝚜𝚝𝚒𝚌𝚔𝚎𝚛 𝚕𝚘𝚌𝚊𝚕𝚕𝚢....' }, { quoted: message });
    }
}
...
function renderTextToPngWithFfmpeg(text) {
    return new Promise((resolve, reject) => {
        ...
        ff.on('close', code => {
            ...
            reject(new Error(Buffer.concat(errors).toString() || `✳️ 𝚏𝚏𝚖𝚙𝚎𝚐 𝚎𝚡𝚒𝚝𝚎𝚍 𝚠𝚒𝚝𝚑 𝚌𝚘𝚍𝚎: ${code}`));
        });
    });
}
