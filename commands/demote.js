...
async function demoteCommand(sock, chatId, mentionedJids, message) {
    try {
        ...
        const demotionMessage = `*⚔️ 𝙶𝚁𝙾𝚄𝙿 𝙳𝙴𝙼𝙾𝚃𝙸𝙾𝙽 ⚔️*\n\n` +
            `👤 𝙳𝚎𝚖𝚘𝚝𝚎𝚍 𝚞𝚜𝚎𝚛: ${userToDemote.length > 1 ? 's' : ''}:\n` +
            `${usernames.map(name => `• ${name}`).join('\n')}\n\n` +
            `👑 𝙳𝚎𝚖𝚘𝚝𝚎𝚍 𝚋𝚢: @${message.key.participant ? message.key.participant.split('@')[0] : message.key.remoteJid.split('@')[0]}\n\n` +
            `📅 𝙳𝚎𝚝𝚎: ${new Date().toLocaleString()}`;
...
async function handleDemotionEvent(sock, groupId, participants, author) {
    try {
        ...
        const demotionMessage = `*⚔️ 𝙶𝚁𝙾𝚄𝙿 𝙳𝙴𝙼𝙾𝚃𝙸𝙾𝙽 ⚔️*\n\n` +
            `👤 𝙳𝚎𝚖𝚘𝚝𝚎𝚍 𝚞𝚜𝚎𝚛: ${participants.length > 1 ? 's' : ''}:\n` +
            `${demotedUsernames.map(name => `• ${name}`).join('\n')}\n\n` +
            `👑 𝙳𝚎𝚖𝚘𝚝𝚎𝚍 𝚋𝚢: ${demotedBy}\n\n` +
            `📅  𝙳𝚎𝚝𝚎: ${new Date().toLocaleString()}`;
        `\n\n` +
